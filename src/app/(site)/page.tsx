...
const featureItems = [
  ...
  { title: "Soğuk zincirle sevkiyat", description: "Ürünlerimizi frigorifik araçlarla sevk ediyoruz.", icon: "shipping" },
] as const;
const qualityItems = [
  { title: "Yerli besi", description: "Hayvanlarımızın beslenmesinde kendi kontrolümüzde üretilen yerli ham maddeleri kullanıyoruz.", icon: "farm" },
...
export default function HomePage() {
  return <div className={`stitch-home ${handwriting.variable}`}>
    <section className="stitch-hero home-hero" aria-labelledby="hero-title">
      ...
      <div className="stitch-container stitch-hero-inner"><div className="stitch-hero-copy">
        <RotatingHeroLabel />
        <h1 id="hero-title">1925’den Bugüne Bir <span className="stitch-hero-accent">Lezzet</span> Deneyimi</h1><p>{homeContent.hero.description}</p>
    ...
    <section className="stitch-section stitch-products" id="urunler" aria-labelledby="stitch-products-title"><div className="stitch-container stitch-split">
      <div className="stitch-copy"><Reveal><p className="stitch-eyebrow">Ürün Yaklaşımımız</p><h2 id="stitch-products-title">{homeContent.products.title}</h2></Reveal>
        ...
        <div className="stitch-feature-list">{featureItems.map(item => <Reveal key={item.title}><div className="stitch-feature"><span className="stitch-feature-mark" aria-hidden="true"><BrandMarkIcon name={item.icon} /></span><div><h3>{item.title}</h3><p>{item.description}</p></div></div></Reveal>)}</div>
    ...
    <section className="stitch-section stitch-quality" aria-labelledby="stitch-quality-title"><div className="stitch-container stitch-split stitch-split-reverse">
      ...
      <div className="stitch-copy"><Reveal><p className="stitch-eyebrow">Üretim &amp; Kalite</p><h2 id="stitch-quality-title">{homeContent.processing.title}</h2></Reveal><Reveal><p className="stitch-description">{homeContent.processing.description}</p></Reveal><div className="stitch-quality-cards">{qualityItems.map(item => <Reveal className="stitch-quality-card" key={item.title}><span className="stitch-quality-mark" aria-hidden="true"><BrandMarkIcon name={item.icon} /></span><h3>{item.title}</h3><p>{item.description}</p></Reveal>)}</div></div>
    </div></section>
