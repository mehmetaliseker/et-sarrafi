...
try {
  ...
  for (const height of [844, 360]) {
    ...
    await key('Enter');
    await waitFor('document.querySelector("#mobile-navigation").dataset.state === "open"');
    await evaluate('document.querySelector("#mobile-navigation a[href=\\"/iletisim\\"]").focus()');
  ...
  await navigate('/');
  report.reducedMotion = await evaluate('({ scrollBehavior: getComputedStyle(document.documentElement).scrollBehavior, transition: getComputedStyle(document.querySelector(".menu-trigger i")).transitionDuration })');
